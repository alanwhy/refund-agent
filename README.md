...
2. 点击“新建测试订单”，选择现有客户、填写商品名和金额，再选择订单特征；
3. 创建成功后记下页面提示中的订单号和客户邮箱；
4. 退出管理员账号，选择“客户入口”，填写该客户邮箱登录；
5. 在“我的订单”点击“申请售后”，确认聊天页已带入订单号，补充原因后提交。

| 订单特征 | 示例金额 | 客户提交退款后的预期结果 |
| --- | --- | --- |
| 正常订单 | ¥399 | 不超过 ¥500 自动退款；超过 ¥500 等待金额审批 |
| 风控订单 | ¥699 | 等待审批，同时保留“风险”和“超过金额上限”两条原因 |
| 支付异常订单 | ¥299 | 通过金额/风控前置规则后，支付未知并进入异常处理 |

金额由管理员输入，范围为 ¥0.01–¥999999.99、最多两位小数。旧版 API 的 `AMOUNT_APPROVAL`
场景仍兼容，但页面不再单独展示；金额是否需要审批始终由真实的 ¥500 规则判断。
...
可以查看全部订单。前端隐藏菜单不是权限边界。每笔订单只显示一个由后端计算的生命周期状态，
例如“已签收”“售后处理中”“等待审批”“人工处理中”或“已退款”；已有工单时按钮变为“查看售后”。

管理员审计页可切换“模型调用”，按一次逻辑调用查看完整消息输入、模型输出、Tool Calls、Token
用量和耗时；“业务事件”保留规则、审批与退款执行记录。审计写入前会递归脱敏密钥、Token、
Authorization、Cookie 等敏感字段。
